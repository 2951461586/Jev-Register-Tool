...
**实测战果**：17 个账号拿到 API Key，`verify_keys.py` 验收 **可用 18 / 不可用 0**
（真打 `api.typesafe.ai` 推理接口）。18 > 17 是因为其中一个账号被重跑过、
...
$PY tools/selftest.py                  # 自测 179 项，离线可跑
...
tools/                     入口脚本
  ...
  tests/                   自测本体（6 个文件 1333 行，179 项，含负对照，**全程离线**）
