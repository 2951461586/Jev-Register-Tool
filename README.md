...
**实测战果**：台账合并视图 233 个地址，其中 **182 个拿到 API Key**。
本轮（邀请制取消后的重构）实跑 **5 个新账号，`verify_keys.py` 真打推理接口
5/5 通过**（`HTTP 200 / model=jev-1.13.0`），单账号关键路径
`login ~6.7s + create_key ~5.0s`。

> ⚠️ 两套口径**行数天然不等**，别拿它们相等当对账判据：
> `result/success.jsonl` 是**账号级**（按邮箱去重 ⇒ 182 行）；
> `result/keys.txt` 是**凭据级**（同账号重跑会拿到第二把 key ⇒ 248 行）。
> 交付凭据以 `keys.txt` 为准。
