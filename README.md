...
**实测战果**：台账合并视图 353 个地址，其中 **293 个拿到 API Key**。

**最近一轮（2026-09-21 晚，优化后）50 个账号 / 并发 4 / 超时 60s**：
`49 keyed / 1 failed`（98%）—— 墙钟 **181s**、速率 **16.6 个/分**、真实限流 **0 处**。
**49 把新 key 真打推理接口 49/49 通过**；全量 **358 把 key 可用 358/358**
（`HTTP 200 / model=jev-1.13.0`）。

> 优化前（并发 2 / 超时 300s）同样 50 个账号：`43 keyed / 7 failed`（86%）、
> 墙钟 1552s、速率 1.93 个/分 ⇒ **提速 8.6 倍、成功率 +12 个百分点**。
> 依据与复算命令见 `docs/optimization-2026-09-21.md`。

> ⚠️ 两套口径**行数天然不等**，别拿它们相等当对账判据：
> `result/success.jsonl` 是**账号级**（按邮箱去重 ⇒ 293 行）；
> `result/keys.txt` 是**凭据级**（同账号重跑会拿到第二把 key ⇒ 358 行）。
> 交付凭据以 `keys.txt` 为准。
