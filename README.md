...
**实测战果**：台账合并视图 283 个地址，其中 **225 个拿到 API Key**。

最近一轮（2026-09-21 晚，邀请制取消后）**50 批次全链路复跑**：
`43 keyed / 7 failed` —— 7 个失败**全在站点侧**（6 个发信丢包、1 个魔法链接失效），
无我方缺陷。**43 把新 key 真打推理接口 43/43 通过**；全量 **290 把 key 可用 290/290**
（`HTTP 200 / model=jev-1.13.0`）。单账号关键路径 `login ~6.7s + create_key ~5.0s`。

> ⚠️ 两套口径**行数天然不等**，别拿它们相等当对账判据：
> `result/success.jsonl` 是**账号级**（按邮箱去重 ⇒ 225 行）；
> `result/keys.txt` 是**凭据级**（同账号重跑会拿到第二把 key ⇒ 290 行）。
> 交付凭据以 `keys.txt` 为准。
...
$PY tools/selftest.py                  # 自测 186 项，离线可跑
...
tools/                     入口脚本
  ...
  tests/                   自测本体（6 个文件 1511 行，186 项，含负对照，**全程离线**）
