...
# 4) 自测（应为 172 项通过；round2 当时是 161，09-21 第三轮 +11）
...
cd /tmp && "$PY" <仓库根>/tools/selftest.py | tail -3   # 末行应为 "通过 172 / 失败 0"（round2 当时 161）

# 9) 批次 C 新增：check() 调用点总数（文档里的 172 是**执行**项数，以运行输出为准）
