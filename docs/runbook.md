...
$PY tools/check_deliverables.py     # ★ 独立复核那 4 份（只读；退出码 0 全绿 / 1 有问题）
```

⚠️ **第二步不是可选项**：`verify_keys.py` 打印的"可用 N / 不可用 0"是**同一进程的自述**。
`check_deliverables.py` 从**产物文件**出发独立验三件事 —— 四份交付物的 **CR 污染**
（`mapfile` / shell 拼参数会带进 `\r`，**不报错**、只是让台账键与 key 值悄悄变错）、
**`apikeys.txt` 与 `keys.txt` 的 key 本体集合恒等**、**与最近一次 `.bak-<时间戳>` 的差集**
（消失的必须能解释）。
⚠️ 它必须**先抽 `apikey_...` 再比集合**：按**整行**比会得到"双向独有 N"的假不一致
...
```bash
$PY tools/selftest.py      # 303 项，含负对照，**全程离线**（不碰网络）
```
...
| `test_ledger` | 工具：key 验收必须重试网络层失败 | 7 | 🔴 `status=0`（**没拿到 HTTP 响应**：SSL 抖动 / DNS / 连接被拒）与 `401`（站点**明确拒绝**）是两类完全不同的失败，混为一类的后果是**交付清单静默缩水**（`keys.txt` 只写 `ok` 的行 ⇒ 一次瞬时抖动就让一把**可用**的 key 从交付物里消失，而屏幕只显示「不可用 1」）。判据是「**有没有拿到 HTTP 响应**」：没拿到 ⇒ 重试 2 次（线性退避 1s/2s）；拿到了 ⇒ 确定性结论，**不重试**。实测（2026-09-22 第 3 批）报 1 把不可用，`elapsed=0.096s`（连接**立即失败**，不是超时）⇒ 手工复验**第 1 次就 200** |
...
| `test_orchestration` | 编排：单号端到端耗时（含静态接线） | 6 | ★ `timings["total"]` 必须**实测**（不能用 `login + create_key` 推导 —— 建邮箱抛异常时 `login` 不写，推导值会让慢账号**静默缺失**、速率虚高）；★ 赋值必须在 `finally` 里（AST 断言 `finalbody`）—— 实测"把赋值补齐到所有出口"的变异**只有静态检查能拦住**（动态用例全绿） |
...
| `test_orchestration` | 工具：批次分析的剔除阈值必须来自源码常量 | 8 | 🔴 剔除"丢包重发"的阈值必须 `import` 自 `src.stages.MAIL_TIMEOUT`，**不许拍一个 30s** —— 实测 `x < 30` 会多剔 3 个"正常但偏慢"的账号，干净均值被**人为压低 1.19s**（7.29 vs 8.48），且**全程零报错**；还必须与日志重发次数**交叉验证**（实测耗时 vs 日志文案，两个独立来源互证）。🔴 交叉验证的**两侧口径必须一致** —— 右侧数的是**全批**日志里的重发次数，左侧就**不能只数 keyed**（有失败账号时会误报；`failed=0` 时两边恰好相等 ⇒ 口径错误被完全掩盖）。⚠️ 脚本必须在 `tools/` 下 —— 放 `exports/`（被 gitignore）⇒ **clone 后本用例必红** |
| `selftest` | 用例登记完整性（AST 元检查） | 1 | 新增 `test_*` 忘记登记 ⇒ **永不执行**，而"通过 N / 失败 0"看起来正常 |

> 合计 **303 项**（32 段 + 1 项入口元检查）。⚠️ 元检查那行**打印在最后一段后面**，
> 所以按输出分段统计时它会被算进**最后一个**用例段（当前是「批次分析的剔除阈值必须来自源码常量」，
> 显示 7 而非 6）——想复算就按"最后一段减 1"处理。
>
