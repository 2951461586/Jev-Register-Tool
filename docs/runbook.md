...
2. 真没有 ⇒ 才按"发送端限流"处理：**拉开投递节奏**（批次之间留间隔），
   不要连续投递 —— 延迟爬升本身就是队列在堆积的信号。
3. 阈值默认已调到 `CONFIRM_TIMEOUT = 300s`（≈ 健康期实测最大延迟的 1.5 倍）。
   需要临时覆盖用 `--confirm-timeout`，但**不要往下调**：
   调小只会把成功申请记成失败，台账里的"失败"会骗人。

> 护栏：`tools/selftest.py` 的 `[编排：确认邮件等待阈值]` 段会把阈值 ≥ 240s
> 钉住，并检查超时文案点明了"迟到可能被误判"。改回 180s 自测立刻失败。
