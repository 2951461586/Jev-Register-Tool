...
```bash
$PY tools/selftest.py      # 294 项，含负对照，**全程离线**（不碰网络）
```
...
| `test_orchestration` | 工具：批次分析的剔除阈值必须来自源码常量 | 6 | 🔴 剔除"丢包重发"的阈值必须 `import` 自 `src.stages.MAIL_TIMEOUT`，**不许拍一个 30s** —— 实测 `x < 30` 会多剔 3 个"正常但偏慢"的账号，干净均值被**人为压低 1.19s**（7.29 vs 8.48），且**全程零报错**；还必须与日志重发次数**交叉验证**（实测耗时 vs 日志文案，两个独立来源互证）。⚠️ 脚本必须在 `tools/` 下 —— 放 `exports/`（被 gitignore）⇒ **clone 后本用例必红** |
| `selftest` | 用例登记完整性（AST 元检查） | 1 | 新增 `test_*` 忘记登记 ⇒ **永不执行**，而"通过 N / 失败 0"看起来正常 |

> 合计 **294 项**（30 段 + 1 项入口元检查）。⚠️ 元检查那行**打印在最后一段后面**，
